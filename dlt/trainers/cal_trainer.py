...
from evaluation.iou import transform, calculate_iou, print_results, get_iou, get_mean_iou

from logger_set import LOG
from utils import masked_l2, masked_cross_entropy, masked_acc, plot_sample, custom_collate_fn
...
class TrainLoopCAL:
    ...
    def train_epoch_CAL(self, epoch):
        ...
        for step, (batch, ids) in enumerate(self.train_dataloader):
            ...
            # print("Geometry: ",batch["geometry"])
            # #print("padding_mask: ", batch["padding_mask"])
            
            # # batch['geometry'] 텐서에서 값이 1보다 큰 요소 찾기
            # greater_than_one = torch.gt(batch['geometry'][:,:,:2], 1)
            # true_count1 = torch.sum(greater_than_one.int())
            
            # greater_than_one = torch.gt(batch['geometry'][:,:,2:4], 1)
            # true_count2 = torch.sum(greater_than_one.int())
            
            # greater_than_one = torch.gt(batch['geometry'][:,:,4:], 1)
            # true_count3 = torch.sum(greater_than_one.int())

            # print("True count1, 2, 3: ", true_count1, true_count2, true_count3)
            ...
            noise = torch.randn(batch['geometry'].shape).to(device) #[batch, 20, 6]
            # print("##########################################################################")
            # print("noise: ", noise)
            # print("##########################################################################")
            ...
            with self.accelerator.accumulate(self.model):
                ...
                loss_batch = 0
                loss = masked_l2(batch['geometry'], geometry_predict, batch['padding_mask'])
                # print("###############################################################################")
                # print("batch[geometry].shape: ", batch["geometry"].shape)
                # print("Geometry_predict.shape: ", geometry_predict.shape)
                loss = loss.mean()
                ...
                # true_box, pred_box = transform(true_geometry, geometry_predict)
                # ious = get_iou(true_box, pred_box)
                # mean_iou = get_mean_iou(ious)
                # print_results(true_box, pred_box)
            ...
            #     wandb.log({"loss": loss.detach().item()}, step=self.global_step)
            #     wandb.log({"mean_iou": mean_iou}, step=self.global_step)  
    ...
    def sample_from_model(self, sample):
        ...
        for i in range(self.diffusion.num_cont_steps)[::-1]:
            ...
            with torch.no_grad():
                # denoise for step t.
                bbox_pred, 
                pred = model(sample, noisy_batch, timesteps=t)
                desc_pred = {
                    'cat': cat_pred,
                }
                # sample
